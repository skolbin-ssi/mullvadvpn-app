...
class MullvadDaemon(vpnService: MullvadVpnService) {
    ...
    fun getDevice(): AccountAndDevice? = getDevice(daemonInterfaceAddress).also {
        _deviceStateUpdates.tryEmit(DeviceState.from(it))
    }

    fun refreshDevice() {
        updateDevice(daemonInterfaceAddress)
        getDevice().also { accountAndDevice ->
            _deviceStateUpdates.tryEmit(DeviceState.from(accountAndDevice))
        }
    }
